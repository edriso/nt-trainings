...
## Who actually answers `/robots.txt`?

A session-12 follow-on, and it changes where the file above lives. One of our
sites moved onto a new CDN configuration, and the `robots.txt` and sitemap files
moved with it, so those two URLs are now answered by the **CDN edge** rather than
by the application behind it.

That is legal, and it is worth understanding why. Google's spec ties `robots.txt`
to a **host, protocol and port**, and requires it at the top level of that host.
It says nothing about which machine produces the bytes. So
`https://example.com/robots.txt` is the contract; whether a theme, an app, or a
CDN config fulfils it is your business, not Google's.
([robots.txt spec](https://developers.google.com/search/docs/crawling-indexing/robots/robots_txt))

Three consequences worth writing down:

| Because the edge answers it | So |
| --- | --- |
| The file is no longer in the app deploy | A code review of the app will not show a robots change. Whoever owns the CDN config owns crawling now |
| Google caches `robots.txt` for up to ~24 hours | A mistake is live for a day even after you fix it. `Cache-Control: max-age` can shorten that window |
| Google follows at least five redirect hops, then treats it as a 404 | An apex to `www` redirect chain on this one file is not a cosmetic problem |

The trade-off to name honestly: this pulls in the *opposite* direction from the
usual advice, which is to move robots rules out of a site-builder's WYSIWYG and
into a templated file in the theme so they show up in a diff. Both are chasing
the same thing: **the file should live somewhere a human reviews before it
ships.** A CDN config in version control satisfies that. A CDN config edited in a
web dashboard does not, and is exactly the WYSIWYG problem wearing a different
hat.

The check after any cutover is the same one either way, and it has to be run from
the public internet rather than from inside the stack:

```bash
# both hostnames, and follow what actually happens
curl -sIL -o /dev/null -w '%{http_code} %{url_effective}\n' https://example.com/robots.txt
curl -sIL -o /dev/null -w '%{http_code} %{url_effective}\n' https://www.example.com/robots.txt
curl -s https://www.example.com/robots.txt | head -20
```

One more habit from the same deploy: the switch went out once the client's team
had gone to sleep. Not because it was expected to break, but because if it did,
